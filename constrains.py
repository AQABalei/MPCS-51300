...
def runFunctionExistCheck(ast, errors):
    funcs = ast['funcs']['funcs']
    numOfRunFunc = 0
    for func in funcs:
        if func['globid'] == 'run':
            if func['ret_type'] != 'int' or 'vdecls' in func:
                errors.append("error: All programs must define exactly one function named “run” which returns an integer (the program exit status) and takes no arguments.")
                return
            numOfRunFunc += 1
    if numOfRunFunc != 1:
        errors.append("error: All programs must define exactly one function named “run” which returns an integer (the program exit status) and takes no arguments.")
